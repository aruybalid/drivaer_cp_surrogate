...
## What the Stage Does
1. Trains a point-based mlp-based regressor (of various kinds, e.g., pure MLP, PointNet (global feature aggregation), on the preprocessed (decimated) data. Uses the model coded in `../utils/model.py`.
...
The following sections of this document describe the command-line arguments for `train.py` and key training concepts.
...
- 
## Human Review
Human reviews the model in `./output` before continuing to the next step. Do not continue without human instruction.