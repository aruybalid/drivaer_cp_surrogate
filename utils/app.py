...
#                     colorscale='Viridis',
#                     cmin=0,
#                     cmax=np.percentile(rel_error, 99),   # use 99th percentile for better contrast
#                     colorbar=dict(title="Rel. Error")
#                 ),
#             )])
#             fig_rel.update_layout(scene=dict(aspectmode='data'), height=500)
#             st.plotly_chart(fig_rel, width='content')

#             # Optional: Add mean relative error metric
#             col1, col2 = st.columns(2)
#             with col1:
#                 st.metric("Mean Relative Error", f"{rel_error.mean():.4f}")
#             with col2:
#                 st.metric("Median Relative Error", f"{np.median(rel_error):.4f}")
